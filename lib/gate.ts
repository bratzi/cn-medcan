 ...
 *
 * Verfahren: Passwort wird nie im Cookie gespeichert. Der Cookie traegt einen
 * HMAC-SHA-256 ueber "gate:<ablaufzeit>", signiert mit SITE_SESSION_SECRET.
 */
...
export async function tokenErzeugen(secret: string): Promise<string> {
  const ablauf = Math.floor(Date.now() / 1000) + GUELTIGKEIT_SEKUNDEN;
  const signatur = await hmac(secret, `gate:${ablauf}`);
  return `${ablauf}.${signatur}`;
}

export async function tokenPruefen(secret: string, token: string | undefined): Promise<boolean> {
  if (!token) return false;
  const [ablaufRoh, signatur] = token.split(".");
  if (!ablaufRoh || !signatur) return false;

  const ablauf = Number(ablaufRoh);
  if (!Number.isFinite(ablauf) || ablauf < Math.floor(Date.now() / 1000)) return false;

  const erwartet = await hmac(secret, `gate:${ablaufRoh}`);
  return gleich(erwartet, signatur);
}
